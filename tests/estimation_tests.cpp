...
static void TestEkfGnssUpdate() {
    ...
    // Push some IMU
    for (int i = 1; i <= 10; ++i) est.AddMeasurement(MakeImu(1'000'000'000LL + i * 10'000'000LL));
...
static void TestEkfDeadReckoningStatus() {
    ...
    for (int i = 1; i <= 20; ++i) est.AddMeasurement(MakeImu(1'100'000'000LL + i * 10'000'000LL));
...
static void TestEkfCovarianceShrinks() {
    ...
    for (int i = 1; i <= 50; ++i) est.AddMeasurement(MakeImu(1'000'000'000LL + i * 10'000'000LL));
...
static void TestEkfBarometerAccepted() {
    ...
    est.AddMeasurement(MakeGnss(1'000'000'000LL, DegToRad(48.0), DegToRad(11.0), 500.0));
    for (int i = 1; i <= 5; ++i) est.AddMeasurement(MakeImu(1'000'000'000LL + i * 10'000'000LL));
...
static void TestEkfBarometerCovarianceShrinks() {
    ...
    est.AddMeasurement(MakeGnss(1'000'000'000LL, DegToRad(48.0), DegToRad(11.0), 500.0));
    for (int i = 1; i <= 30; ++i) est.AddMeasurement(MakeImu(1'000'000'000LL + i * 10'000'000LL));
...
static void TestEkfMagnetometerAccepted() {
    ...
    const auto report = est.AddMeasurement(MakeMag(1'060'000'000LL, Eigen::Vector3d(0.0, 2.0e-5, -4.3e-5)));
    assert(report.result == EstimatorUpdateResult::Accepted);
...
static void TestNavigationSystemPipeline() {
    ...
    struct Observer : INavigationObserver {
        ...
        void OnNavigationState(std::shared_ptr<const NavigationState>) override { received = true; }
    } obs(state_received);
    ...
    for (int i = 1; i <= 5; ++i) nav.Pipeline().Push(MakeImu(1'000'000'000LL + i * 10'000'000LL));
