...
static json jvec3() { return {{"type", "array"}, {"items", {{"type", "number"}}}, {"minItems", 3}, {"maxItems", 3}}; }
...
static json WriteVec3(const Eigen::Vector3d& v) { return json::array({v.x(), v.y(), v.z()}); }

static json WriteMat3(const Eigen::Matrix3d& m) {
    json a = json::array();
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c) a.push_back(m(r, c));
    return a;
}

static Eigen::Matrix3d ReadMat3(const json& j, const std::string& key) {
    if (!j.contains(key) || !j[key].is_array() || j[key].size() < 9) return Eigen::Matrix3d::Identity();
    const auto& a = j[key];
    Eigen::Matrix3d m;
    for (int r = 0; r < 3; ++r)
        for (int c = 0; c < 3; ++c) m(r, c) = a[r * 3 + c].get<double>();
    return m;
}
...
static NavigationSystemConfig NavFromJson(const json& j) {
    ...
    if (j.contains("backend")) {
        ...
        if (b == "UKF")
            ...
            cfg.backend = EstimatorBackendChoice::Ceres;
        else if (b == "GTSAM")
            cfg.backend = EstimatorBackendChoice::Gtsam;
        else
    ...
    if (j.contains("gnss")) {
        ...
        cfg.gnss.mode =
            (mode == "TightlyCoupled") ? GnssIntegrationMode::TightlyCoupled : GnssIntegrationMode::LooselyCoupled;
        ...
        if (jg.contains("tightly_coupled")) {
            ...
            if (tc.contains("base_pseudorange_sigma_m"))
                cfg.gnss.tightly_coupled.base_pseudorange_sigma_m = tc["base_pseudorange_sigma_m"];
            if (tc.contains("pseudorange_noise_floor_m"))
                cfg.gnss.tightly_coupled.pseudorange_noise_floor_m = tc["pseudorange_noise_floor_m"];
            if (tc.contains("base_doppler_sigma_mps"))
                cfg.gnss.tightly_coupled.base_doppler_sigma_mps = tc["base_doppler_sigma_mps"];
            if (tc.contains("innovation_gate_sigma"))
                cfg.gnss.tightly_coupled.innovation_gate_sigma = tc["innovation_gate_sigma"];
        }
    }
    ...
    if (j.contains("wheel_odometry")) {
        ...
        if (jw.contains("sigma_angular_radps"))
            cfg.wheel_odometry.options.sigma_angular_radps = jw["sigma_angular_radps"];
    ...
    if (j.contains("range_finder")) {
        ...
        if (jr.contains("terrain_elevation_m"))
            cfg.range_finder.options.terrain_elevation_m = jr["terrain_elevation_m"];
    ...
    if (j.contains("optical_flow")) {
        ...
        if (jo.contains("fallback_altitude_m"))
            cfg.optical_flow.options.fallback_altitude_m = jo["fallback_altitude_m"];
...
SessionConfig SessionConfig::FromPath(const std::string& path) {
    if (!std::filesystem::exists(path)) throw std::invalid_argument("Config file not found: " + path);
    std::ifstream f(path);
...
bool SessionConfig::Validate(std::string& error) const {
    if (dataset.path.empty()) {
        error = "dataset.path is empty";
        return false;
    }
    if (ipc.socket_path.empty()) {
        error = "ipc.socket_path is empty";
        return false;
    }
    if (ipc.max_clients < 1) {
        error = "ipc.max_clients must be >= 1";
        return false;
    }
    if (playback_speed < 0.0) {
        error = "playback_speed must be >= 0 (0 = as fast as possible)";
        return false;
    }
    return true;
}
