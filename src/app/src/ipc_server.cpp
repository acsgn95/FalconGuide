...
IpcServer::IpcServer(IpcConfig cfg, CommandHandler handler)
...
void IpcServer::BroadcastNavState(const core::NavigationState& state) {
  ...
  const double roll_rad  = std::atan2(2*(q.w()*q.x()+q.y()*q.z()),
                                       1-2*(q.x()*q.x()+q.y()*q.y()));
  const double pitch_rad = std::asin( std::clamp(2*(q.w()*q.y()-q.z()*q.x()), -1.0, 1.0));
  const double yaw_rad   = std::atan2(2*(q.w()*q.z()+q.x()*q.y()),
                                       1-2*(q.y()*q.y()+q.z()*q.z()));
  ...
  int64_t ts_ns = 0;
  if (state.timestamp.has_steady)
    ts_ns = state.timestamp.steady.nanoseconds_since_epoch().count();
  ...
  Broadcast({
    ...
    {"vel_enu_mps",  json::array({state.velocity_enu_mps.x(), state.velocity_enu_mps.y(), state.velocity_enu_mps.z()})},
...
void IpcServer::BroadcastError(const std::string& message) {
  Broadcast({{"event","error"},{"message",message}});
}

void IpcServer::BroadcastSchema(const json& schema) {
  Broadcast({{"event","schema"},{"schema",schema}});
}
