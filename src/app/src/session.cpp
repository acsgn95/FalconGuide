...
FalconGuideSession::FalconGuideSession(SessionConfig cfg)
    : cfg_(std::move(cfg)) {
  // Build nav system
  ...
  ipc_server_ = std::make_unique<IpcServer>(
      cfg_.ipc,
      [this](const nlohmann::json& cmd, int fd) { HandleCommand(cmd, fd); });

  SetStatus(Status::Configured);
}

FalconGuideSession::~FalconGuideSession() {
  Stop();
}
...
bool FalconGuideSession::Start() {
  ...
  log::Get()->info("Session started — dataset: {}, socket: {}",
                              cfg_.dataset.path, cfg_.ipc.socket_path);
  return true;
}
...
void FalconGuideSession::ReplayLoop() {
    ...
    if (cfg_.playback_speed > 0.0) {
      ...
      if (ts.has_steady) {
        ...
        if (prev_meas_ns && cur_ns > *prev_meas_ns) {
          ...
          const auto sleep_ns = static_cast<int64_t>(
              static_cast<double>(dt_meas_ns) / cfg_.playback_speed);
  ...
  if (running_.load()) {
    ...
    log::Get()->info("Dataset replay completed ({} measurements).",
                                dataset_reader_->MeasurementsRead());
  }
}

// ── INavigationObserver ───────────────────────────────────────────────────────

void FalconGuideSession::OnNavigationState(
    std::shared_ptr<const core::NavigationState> state) {
  if (state) ipc_server_->BroadcastNavState(*state);
}

void FalconGuideSession::OnEstimatorReset() {
  ipc_server_->BroadcastStatus("reset");
}
...
FalconGuideSession::Status FalconGuideSession::GetStatus() const {
  return status_.load();
}
...
std::string FalconGuideSession::GetStatusString() const {
  switch (status_.load()) {
    ...
    case Status::Configured: return "configured";
