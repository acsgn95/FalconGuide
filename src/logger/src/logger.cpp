...
void Init(const LoggerConfig& config) {
    ...
    if (config.file_path) {
        ...
        auto file_sink =
            std::make_shared<spdlog::sinks::rotating_file_sink_mt>(*config.file_path, max_bytes, config.max_files);
        // File sink: plain pattern (no ANSI colours in log files)
        const std::string file_pattern = "[%Y-%m-%d %H:%M:%S.%e] [" + config.name + "] [%-8l] [t:%t] %v";
    ...
    if (config.network) {
        ...
        const std::string net_pattern = "[%Y-%m-%d %H:%M:%S.%e] [" + config.name + "] [%-8l] [t:%t] %v";
