 ...
 */
template <typename Measurement> class TimeOrderedBuffer {
public:
  ...
  /// element.
  explicit TimeOrderedBuffer(std::size_t max_size = 0,
                             bool allow_out_of_order_insert = true)
      : max_size_(max_size),
        allow_out_of_order_insert_(allow_out_of_order_insert) {}
  ...
  BufferInsertResult Insert(Measurement measurement) {
    ...
    const TimestampOrdering latest_ordering =
        CompareTimestamps(GetTimestamp(measurements_.back()), timestamp);
    if (latest_ordering == TimestampOrdering::Before ||
        latest_ordering == TimestampOrdering::Equal) {
      measurements_.push_back(std::move(measurement));
    ...
    for (auto iterator = measurements_.begin(); iterator != measurements_.end();
         ++iterator) {
      const TimestampOrdering ordering =
          CompareTimestamps(timestamp, GetTimestamp(*iterator));
      if (ordering == TimestampOrdering::NotComparable) {
        return BufferInsertResult::RejectedNotComparable;
      }
      if (ordering == TimestampOrdering::Before ||
          ordering == TimestampOrdering::Equal) {
  ...
  [[nodiscard]] std::vector<Measurement> PopUntil(const Timestamp &timestamp,
                                                  bool include_equal = true) {
    std::vector<Measurement> result;
    while (!measurements_.empty()) {
      const TimestampOrdering ordering =
          CompareTimestamps(GetTimestamp(measurements_.front()), timestamp);
      if (ordering == TimestampOrdering::NotComparable) {
        break;
      }
      const bool should_pop =
          ordering == TimestampOrdering::Before ||
          (include_equal && ordering == TimestampOrdering::Equal);
      if (!should_pop) {
        break;
      }
