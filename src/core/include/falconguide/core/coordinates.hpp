...
inline constexpr double kFirstEccentricitySquared =
...
[[nodiscard]] inline Vec3<EcefFrame> LlaToEcef(const Lla &lla) {
  ...
  const double prime_vertical_radius =
      ...
      std::sqrt(1.0 - wgs84::kFirstEccentricitySquared * sin_lat * sin_lat);
  ...
  const double z =
      (prime_vertical_radius * (1.0 - wgs84::kFirstEccentricitySquared) +
       lla.altitude_m) *
      sin_lat;
...
[[nodiscard]] inline Lla EcefToLla(const Vec3<EcefFrame> &ecef) {
  ...
  const double theta =
      std::atan2(z * wgs84::kSemiMajorAxisM, p * wgs84::kSemiMinorAxisM);
  const double sin_theta = std::sin(theta);
  const double cos_theta = std::cos(theta);

  const double latitude = std::atan2(
      z + wgs84::kSecondEccentricitySquared * wgs84::kSemiMinorAxisM *
              sin_theta * sin_theta * sin_theta,
      p - wgs84::kFirstEccentricitySquared * wgs84::kSemiMajorAxisM *
...
/// coordinates.
[[nodiscard]] inline Eigen::Matrix3d EcefToEnuRotation(double latitude_rad,
...
public:
  ...
  explicit LocalTangentPlane(const Lla &origin_lla)
      : origin_lla_(origin_lla), origin_ecef_m_(LlaToEcef(origin_lla)),
        ecef_to_enu_rotation_(EcefToEnuRotation(origin_lla.latitude_rad,
                                                origin_lla.longitude_rad)) {}
  ...
  [[nodiscard]] const Vec3<EcefFrame> &origin_ecef_m() const {
    return origin_ecef_m_;
  }
  /// @brief Returns the ECEF-to-ENU rotation matrix at the origin.
  [[nodiscard]] const Eigen::Matrix3d &ecef_to_enu_rotation() const {
    return ecef_to_enu_rotation_;
  }

  /// @brief Converts an ECEF point to local ENU coordinates.
  [[nodiscard]] Vec3<EnuFrame>
  EcefToEnu(const Vec3<EcefFrame> &point_ecef_m) const {
    const Eigen::Vector3d delta_ecef =
        point_ecef_m.eigen() - origin_ecef_m_.eigen();
    return Vec3<EnuFrame>(ecef_to_enu_rotation_ * delta_ecef);
  }

  /// @brief Converts a local ENU point to ECEF coordinates.
  [[nodiscard]] Vec3<EcefFrame>
  EnuToEcef(const Vec3<EnuFrame> &point_enu_m) const {
    return Vec3<EcefFrame>(origin_ecef_m_.eigen() +
                           ecef_to_enu_rotation_.transpose() *
                               point_enu_m.eigen());
  }
