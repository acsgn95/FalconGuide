...
struct CameraCalibration {
  ...
  Transform<CameraFrame, BodyFrame>
      camera_to_body; ///< Camera to body transform.
};
...
struct CameraConfig {
  ...
  bool hardware_synchronized{
      false}; ///< True when camera timestamps are hardware synchronized.
};
...
struct GeoReferenceImageMeasurement {
  ...
  std::optional<Vec3<EcefFrame>>
      approximate_position_ecef_m; ///< Optional approximate search position.
  std::optional<double>
      ground_sample_distance_m;        ///< Optional ground sample distance.
