...
struct MagnetometerCalibration {
    ...
    Eigen::Matrix3d soft_iron_scale{Eigen::Matrix3d::Identity()};  ///< Soft-iron scale matrix.
    Eigen::Matrix3d misalignment{Eigen::Matrix3d::Identity()};     ///< Sensor-axis misalignment matrix.
    NoiseDensity magnetic_field_noise;                             ///< Magnetic-field noise model.
    Transform<MagnetometerFrame, BodyFrame> magnetometer_to_body;  ///< Magnetometer to body transform.
};
...
struct BarometerMeasurement {
    ...
    std::optional<double> temperature_celsius;                 ///< Optional ambient or sensor temperature.
...
struct AirspeedCalibration {
    double differential_pressure_bias_pa{0.0};  ///< Additive differential-pressure bias.
    double differential_pressure_scale{1.0};    ///< Differential-pressure scale factor.
...
struct AirspeedMeasurement {
    Timestamp timestamp;                                       ///< Sample timestamp.
    AirspeedType type{AirspeedType::Unknown};                  ///< Type of airspeed value supplied.
    double airspeed_mps{0.0};                                  ///< Airspeed in meters per second.
    std::optional<double> differential_pressure_pa;            ///< Optional differential pressure.
