...
struct SatelliteId {
  GnssConstellation constellation{
      GnssConstellation::Unknown}; ///< Satellite constellation.
...
struct GnssRawObservation {
  ...
  std::optional<double>
      carrier_phase_cycles;         ///< Optional carrier phase in cycles.
  std::optional<double> doppler_hz; ///< Optional Doppler shift in hertz.
  std::optional<double>
      carrier_to_noise_density_dbhz; ///< Optional C/N0 in dB-Hz.
  std::optional<double>
      lock_time_s; ///< Optional continuous tracking lock time.
  MeasurementValidity validity{
      MeasurementValidity::Valid}; ///< Observation validity state.
};

/// @brief Timestamped batch of raw GNSS observations.
struct GnssObservationEpoch {
  Timestamp timestamp; ///< Epoch timestamp.
  std::vector<GnssRawObservation>
      observations; ///< Raw observations visible at this epoch.
  MeasurementValidity validity{
      MeasurementValidity::Valid}; ///< Epoch validity state.
};
...
struct GnssSolution {
  ...
  Eigen::Matrix3d velocity_covariance_ecef_m2ps2{
      Eigen::Matrix3d::Zero()};             ///< ECEF velocity covariance.
  GnssFixType fix_type{GnssFixType::NoFix}; ///< Receiver fix type.
  std::optional<double>
      horizontal_dop; ///< Optional horizontal dilution of precision.
  std::optional<double>
      vertical_dop; ///< Optional vertical dilution of precision.
  MeasurementValidity validity{
      MeasurementValidity::Valid}; ///< Solution validity state.
};

/// @brief GNSS antenna lever-arm and phase-center calibration.
struct GnssAntennaCalibration {
  Transform<GnssAntennaFrame, BodyFrame>
      antenna_to_body; ///< Antenna-frame to body-frame transform.
  Vec3<GnssAntennaFrame>
      phase_center_offset_m; ///< Phase-center offset in antenna frame.
};
...
struct SatelliteState {
  ...
  Vec3<EcefFrame>
      position_ecef_m; ///< ECEF satellite position at transmit time.
  Vec3<EcefFrame>
      velocity_ecef_mps; ///< ECEF satellite velocity at transmit time.
  ...
  double tropospheric_delay_m{
      0.0}; ///< Tropospheric delay correction in meters.

  // Set false if the navigation message marks this SV unhealthy.
  bool healthy{
      true}; ///< False when navigation data marks the satellite unhealthy.
};
...
struct GnssTightlyCoupledEpoch {
  Timestamp timestamp; ///< Epoch timestamp.
  std::vector<GnssTightlyCoupledObservation>
      sv_observations; ///< Paired observations and satellite states.
  MeasurementValidity validity{
      MeasurementValidity::Valid}; ///< Epoch validity state.
};
