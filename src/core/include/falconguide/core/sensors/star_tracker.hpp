...
struct StarTrackerMeasurement {
  ...
  double position_sigma_rad{
      0.0}; ///< One-sigma horizontal position uncertainty in radians.
  ...
  // empty.
  Eigen::Vector2d pitch_roll_sigma_rad{
      Eigen::Vector2d::Ones() * 1e6}; ///< One-sigma [pitch, roll] uncertainty.

  // ── Quality ────────────────────────────────────────────────────────────────
  std::optional<std::uint16_t>
      stars_tracked; ///< Optional number of stars used by the solver.
...
struct StarTrackerCalibration {
  Transform<StarTrackerFrame, BodyFrame>
      star_tracker_to_body;      ///< Star-tracker to body transform.
