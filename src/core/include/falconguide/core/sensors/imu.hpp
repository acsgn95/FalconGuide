...
struct ImuCalibration {
    Vec3<ImuFrame> accelerometer_bias_mps2{};                                 ///< Accelerometer additive bias in m/s^2.
    Vec3<ImuFrame> gyroscope_bias_radps{};                                    ///< Gyroscope additive bias in rad/s.
    Eigen::Matrix3d accelerometer_scale{Eigen::Matrix3d::Identity()};         ///< Accelerometer scale correction.
    Eigen::Matrix3d gyroscope_scale{Eigen::Matrix3d::Identity()};             ///< Gyroscope scale correction.
    Eigen::Matrix3d accelerometer_misalignment{Eigen::Matrix3d::Identity()};  ///< Accelerometer axis misalignment.
    Eigen::Matrix3d gyroscope_misalignment{Eigen::Matrix3d::Identity()};      ///< Gyroscope axis misalignment.
    NoiseDensity accelerometer_noise;                                         ///< Accelerometer noise model.
    NoiseDensity gyroscope_noise;                                             ///< Gyroscope noise model.
    Transform<ImuFrame, BodyFrame> imu_to_body;  ///< Rigid transform from IMU frame to body frame.
};
...
struct ImuMeasurement {
    ...
    Vec3<ImuFrame> angular_rate_radps;                         ///< Angular rate measured by gyroscopes in rad/s.
