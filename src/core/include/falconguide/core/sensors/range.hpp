...
struct RadarAltimeterCalibration {
    Transform<RadarAltimeterFrame, BodyFrame> radar_altimeter_to_body;  ///< Sensor to body transform.
    ...
    RadarAltimeterMode mode{RadarAltimeterMode::Unknown};               ///< Altimeter operating mode.
};
...
struct RadarAltimeterMeasurement {
    ...
    std::optional<double> vertical_velocity_mps;                    ///< Optional vertical velocity estimate.
    ...
    RadarAltimeterSurface surface{RadarAltimeterSurface::Unknown};  ///< Classified reflecting surface.
...
struct RangeFinderCalibration {
    Transform<RangeFinderFrame, BodyFrame> range_finder_to_body;  ///< Sensor to body transform.
...
struct RangeFinderMeasurement {
    ...
    std::optional<double> signal_quality;                      ///< Optional normalized or device-specific quality.
    RangeFinderType type{RangeFinderType::Unknown};            ///< Range-finder technology.
    MeasurementValidity validity{MeasurementValidity::Valid};  ///< Source validity state.
};
...
struct OpticalFlowCalibration {
    Transform<OpticalFlowFrame, BodyFrame> optical_flow_to_body;  ///< Sensor to body transform.
...
struct OpticalFlowMeasurement {
    Timestamp timestamp;                                       ///< Sample timestamp.
    double integrated_flow_x_rad{0.0};                         ///< Integrated image flow around x in radians.
    double integrated_flow_y_rad{0.0};                         ///< Integrated image flow around y in radians.
    std::optional<double> integration_time_s;                  ///< Optional integration interval.
    std::optional<double> ground_distance_m;                   ///< Optional distance to ground used for scaling.
    std::optional<Vec3<BodyFrame>> angular_rate_body_radps;    ///< Optional body angular-rate compensation.
...
struct DvlMeasurement {
    ...
    Eigen::Matrix3d velocity_covariance_m2ps2{Eigen::Matrix3d::Zero()};  ///< Velocity covariance.
...
struct EchoSounderCalibration {
    Transform<EchoSounderFrame, BodyFrame> echo_sounder_to_body;  ///< Echo-sounder to body transform.
    double range_bias_m{0.0};                                     ///< Additive range bias.
    double range_scale{1.0};                                      ///< Multiplicative range scale.
    NoiseDensity range_noise;                                     ///< Range noise model.
};
