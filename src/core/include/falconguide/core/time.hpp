...
public:
  ...
  [[nodiscard]] constexpr std::chrono::nanoseconds nanoseconds() const {
    return value_;
  }
  /// @brief Returns the duration as floating-point seconds.
  [[nodiscard]] double seconds() const {
    return static_cast<double>(value_.count()) / 1'000'000'000.0;
  }
...
public:
  ...
  /// epoch.
  explicit constexpr MonotonicTime(std::chrono::nanoseconds value)
      : value_(value) {}
...
public:
  ...
  constexpr GpsTime(std::int32_t week, double seconds_of_week)
      : week_(week), seconds_of_week_(seconds_of_week) {}
  ...
  [[nodiscard]] constexpr double seconds_of_week() const {
    return seconds_of_week_;
  }
