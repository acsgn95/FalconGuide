...
ReaderCapabilities CsvMeasurementReader::Capabilities() const {
    return ReaderCapabilities::Imu | ReaderCapabilities::Gnss | ReaderCapabilities::Barometer |
...
std::optional<core::Timestamp> CsvMeasurementReader::StartTime() const { return start_time_; }

std::optional<core::Timestamp> CsvMeasurementReader::EndTime() const { return end_time_; }
...
ReadOutcome CsvMeasurementReader::ParseLine(const std::string& line) {
    ...
    try {
        ...
        if (type == "IMU" && tokens.size() >= 8) {
            ...
            imu.specific_force_mps2 =
                core::Vec3<core::ImuFrame>(ToDouble(tokens[2]), ToDouble(tokens[3]), ToDouble(tokens[4]));
            imu.angular_rate_radps =
                core::Vec3<core::ImuFrame>(ToDouble(tokens[5]), ToDouble(tokens[6]), ToDouble(tokens[7]));
        ...
        if (type == "GNSS" && tokens.size() >= 10) {
            ...
            gnss.velocity_ecef_mps = core::Vec3<core::EcefFrame>(R_enu_to_ecef * Eigen::Vector3d(ve, vn, vu));
        ...
        if (type == "MAG" && tokens.size() >= 5) {
            ...
            mag.magnetic_field_tesla =
                core::Vec3<core::MagnetometerFrame>(ToDouble(tokens[2]), ToDouble(tokens[3]), ToDouble(tokens[4]));
