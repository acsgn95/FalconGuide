...
/**
 * @file nmea_gga.hpp
 * @brief NMEA GGA parsing and FalconGuide GGA sentence generation.
 */
...
/// @brief Parses an NMEA latitude or longitude field into radians.
[[nodiscard]] inline std::optional<double>
ParseNmeaLatitudeLongitude(std::string_view value, std::string_view hemisphere,
                           bool is_latitude) {
...
/// @brief Formats radians as an NMEA latitude or longitude field.
[[nodiscard]] inline std::string FormatNmeaLatitudeLongitude(double radians,
                                                             bool is_latitude) {
  ...
  const double minutes =
      (absolute_degrees - static_cast<double>(degrees)) * 60.0;
  const int degree_width = is_latitude ? 2 : 3;

  std::ostringstream stream;
  stream << std::setw(degree_width) << std::setfill('0') << degrees
         << std::fixed << std::setprecision(7) << std::setw(10)
         << std::setfill('0') << minutes;
  return stream.str();
}

/// @brief Converts a GGA fix-quality field to GnssFixType.
[[nodiscard]] inline core::GnssFixType
ParseGgaFixType(const std::string &field) {
  if (field == "1") {
...
/// @brief Converts NavigationStatus to a GGA fix-quality field.
[[nodiscard]] inline std::string
FormatGgaFixType(core::NavigationStatus status) {
  if (status == core::NavigationStatus::Nominal ||
      status == core::NavigationStatus::Degraded ||
...
/// @brief Parses a validated GGA sentence into a GNSS solution.
[[nodiscard]] inline std::optional<core::GnssSolution>
ParseGga(const NmeaSentence &sentence) {
  ...
  const double altitude_m =
      sentence.fields[8].empty() ? 0.0 : std::stod(sentence.fields[8]);
  core::GnssSolution solution;
  solution.position_ecef_m =
      core::LlaToEcef(core::Lla{*latitude, *longitude, altitude_m});
  solution.fix_type = ParseGgaFixType(sentence.fields[5]);
  solution.horizontal_dop = sentence.fields[7].empty()
                                ? std::optional<double>{}
                                : std::stod(sentence.fields[7]);
  solution.validity = solution.fix_type == core::GnssFixType::NoFix
                          ? core::MeasurementValidity::Unknown
                          : core::MeasurementValidity::Valid;
  return solution;
}

/// @brief Parses a raw GGA line into a GNSS solution.
[[nodiscard]] inline std::optional<core::GnssSolution>
ParseGgaLine(std::string_view line) {
...
/// @brief Writes a FalconGuide GGA sentence from a navigation state.
[[nodiscard]] inline std::string WriteGga(const core::NavigationState &state,
                                          std::string_view utc_time = "") {
  const core::Lla lla = core::EcefToLla(state.position_ecef_m);
  const std::string latitude =
      FormatNmeaLatitudeLongitude(lla.latitude_rad, true);
  const std::string longitude =
      FormatNmeaLatitudeLongitude(lla.longitude_rad, false);
  ...
  if (state.quality.horizontal_accuracy_m) {
    hdop << std::fixed << std::setprecision(1)
         << *state.quality.horizontal_accuracy_m;
  }

  const std::vector<std::string> fields{std::string(utc_time),
                                        latitude,
