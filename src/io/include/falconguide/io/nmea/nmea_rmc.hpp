...
RmcStatusChar(core::NavigationStatus status) noexcept {
  switch (status) {
...
ParseRmc(const NmeaSentence &sentence) {
  ...
  solution.position_ecef_m =
      core::LlaToEcef(core::Lla{*latitude, *longitude, 0.0});
...
[[nodiscard]] inline std::optional<core::GnssSolution>
ParseRmcLine(std::string_view line) {
...
[[nodiscard]] inline std::string WriteRmc(const core::NavigationState &state,
  ...
  // 360).
  double course_deg = core::RadToDeg(
      std::atan2(state.velocity_enu_mps.x(), state.velocity_enu_mps.y()));
  if (course_deg < 0.0) {
    course_deg += 360.0;
  }
