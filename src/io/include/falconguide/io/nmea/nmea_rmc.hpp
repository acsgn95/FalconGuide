...
// RMC status: 'A' = active (valid fix), 'V' = void (no fix).
...
// Parse a GnssSolution from an RMC sentence.
// RMC provides: position, speed over ground (knots), course over ground, date.
// Velocity in ECEF is not recoverable without heading — only speed/course set.
[[nodiscard]] inline std::optional<core::GnssSolution> ParseRmc(const NmeaSentence& sentence) {
  // Fields: hhmmss.ss, status, lat, N/S, lon, E/W, speed_knots, course, ddmmyy, mag_var, E/W, [mode]
  ...
  const std::optional<double> latitude =
      ParseNmeaLatitudeLongitude(sentence.fields[2], sentence.fields[3], true);
  const std::optional<double> longitude =
      ParseNmeaLatitudeLongitude(sentence.fields[4], sentence.fields[5], false);
...
// Build an RMC sentence from a NavigationState.
// utc_time: "hhmmss.ss", date: "ddmmyy" — caller provides these from system/GPS time.
[[nodiscard]] inline std::string WriteRmc(const core::NavigationState& state,
  ...
  const double speed_enu_mps =
      std::sqrt(state.velocity_enu_mps.x() * state.velocity_enu_mps.x() +
  ...
  // Course over ground: angle from North (ENU Y-axis), clockwise, degrees [0, 360).
