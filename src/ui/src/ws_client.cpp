...
std::string WsClient::Base64(const uint8_t* data, std::size_t len) {
    ...
    for (std::size_t i = 0; i < len; i += 3) {
        ...
        out += kAlph[(v>>18)&63]; out += kAlph[(v>>12)&63];
...
bool WsClient::RecvFrame(int fd, std::string& out) {
    auto recv1 = [&](uint8_t& b) {
        return sock_recv_timeout(fd, &b, 1, 5000) == 1;
    };
    ...
    if (plen == 126) {
        uint8_t b[2]; if (!recv1(b[0]) || !recv1(b[1])) return false;
        plen = (uint64_t(b[0]) << 8) | b[1];
    } else if (plen == 127) {
        uint8_t b[8];
        for (auto& x : b) if (!recv1(x)) return false;
        plen = 0; for (int i=0;i<8;i++) plen = (plen<<8)|b[i];
    }

    uint8_t mask[4] = {};
    if (masked) for (auto& b : mask) if (!recv1(b)) return false;
...
void WsClient::Disconnect() {
    ...
    if (socket_fd_ >= 0) { sock_close(socket_fd_); socket_fd_ = -1; }
