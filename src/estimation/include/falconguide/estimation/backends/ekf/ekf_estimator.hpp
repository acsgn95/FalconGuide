...
struct EkfOptions {
  ...
  bool enable_gnss_clock_state{false};

  // Set true to add StateSegmentId::BaroBias.
  bool enable_baro_bias_state{false};
};
...
// star tracker, DVL, …).  The engine dispatches each incoming measurement to the
// first model that reports CanHandle() == true.
//
...
class EkfEstimator : public INavigationEstimator {
 public:
  ...
  // Register a measurement model.  Models are tried in registration order on
  // every incoming non-IMU measurement.
