...
struct CeresOptions {
  ...
  double keyframe_min_distance_m{
      0.5}; ///< Create a keyframe after this translation.
  double keyframe_min_rotation_rad{
      0.1}; ///< Create a keyframe after this rotation.

  // Solver parameters.
  SolverStrategy solver_strategy{
      SolverStrategy::IteratedFull}; ///< Solver iteration policy.
  ...
  // Robustification.
  LossFunctionType gnss_loss{
      LossFunctionType::Huber}; ///< Robust loss for GNSS factors.
  LossFunctionType vision_loss{
      LossFunctionType::Huber};     ///< Robust loss for vision factors.
