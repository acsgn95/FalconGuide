...
/**
 * @file ceres_cost_functions.hpp
 * @brief AutoDiff cost functions used by the Ceres sliding-window backend.
 */
...
static constexpr double kGravityMps2 = 9.80665;                     ///< Standard gravity magnitude.
static const Eigen::Vector3d kGravityEnu{0.0, 0.0, -kGravityMps2};  ///< Gravity vector in ENU.

// ── Helpers
// ───────────────────────────────────────────────────────────────────

/// @brief Quaternion exponential map templated for Ceres Jet support.
template <typename T>
static Eigen::Quaternion<T> QuatExp(const Eigen::Matrix<T, 3, 1> &phi) {
    const T angle = phi.norm();
    if (angle < T(1e-10)) return {T(1), phi(0) * T(0.5), phi(1) * T(0.5), phi(2) * T(0.5)};
    const T s = ceres::sin(T(0.5) * angle) / angle;
    return {ceres::cos(T(0.5) * angle), s * phi(0), s * phi(1), s * phi(2)};
}

/// @brief Approximate quaternion log residual for near-identity rotations.
template <typename T>
...
struct ImuPreintegrationCost {
    /// @brief Stores the preintegrated IMU factor data.
    explicit ImuPreintegrationCost(ImuPreintegration preint) : preint_(std::move(preint)) {}

    template <typename T>
    bool operator()(const T *pose_i, const T *vel_i, const T *bias_i, const T *pose_j, const T *vel_j, const T *bias_j,
                    T *residual) const {
        // Unpack pose_i
        Eigen::Matrix<T, 3, 1> p_i(pose_i[0], pose_i[1], pose_i[2]);
        Eigen::Quaternion<T> q_i(pose_i[6], pose_i[3], pose_i[4],
                                 pose_i[5]);  // w,x,y,z
        q_i.normalize();
        ...
        Eigen::Matrix<T, 3, 1> dp_corr =
            preint_.delta_p.template cast<T>() + Jdp_dba.block(0, 0, 3, 3) * dba + Jdp_dbg.block(0, 0, 3, 3) * dbg;
        ...
        Eigen::Matrix<T, 3, 1> dv_corr =
            preint_.delta_v.template cast<T>() + Jdp_dba.block(6, 0, 3, 3) * dba + Jdp_dbg.block(6, 0, 3, 3) * dbg;
        ...
        Eigen::Matrix<T, 3, 1> r_p = R_i.transpose() * (p_j - p_i - v_i * dt - T(0.5) * g_enu * dt * dt) - dp_corr;

        // Rotation residual: 2*[ΔqCorr^{-1} ⊗ q_i^{-1} ⊗ q_j]_xyz
        Eigen::Quaternion<T> dq_hat(T(preint_.delta_q.w()), T(preint_.delta_q.x()), T(preint_.delta_q.y()),
                                    T(preint_.delta_q.z()));
        ...
        Eigen::Matrix<T, 3, 1> r_v = R_i.transpose() * (v_j - v_i - g_enu * dt) - dv_corr;
    ...
    /// @brief Creates a Ceres AutoDiff cost function for this factor.
    ...
    ImuPreintegration preint_;  ///< Preintegrated IMU data captured by the factor.
};
...
struct GnssPositionCost {
    /// @brief Builds a GNSS position residual from ENU measurement and
    /// covariance.
    GnssPositionCost(Eigen::Vector3d meas_enu_m, Eigen::Matrix3d cov_m2) : meas_(std::move(meas_enu_m)) {
    ...
    Eigen::Vector3d meas_;       ///< Measured ENU position.
    Eigen::Matrix3d sqrt_info_;  ///< Square-root information matrix.
};
...
struct GnssVelocityCost {
    /// @brief Builds a GNSS velocity residual from ENU measurement and
    /// covariance.
    GnssVelocityCost(Eigen::Vector3d meas_enu_mps, Eigen::Matrix3d cov_m2ps2) : meas_(std::move(meas_enu_mps)) {
    ...
    Eigen::Vector3d meas_;       ///< Measured ENU velocity.
    Eigen::Matrix3d sqrt_info_;  ///< Square-root information matrix.
};
...
struct BiasRandomWalkCost {
    /// @brief Builds a bias random-walk residual.
    BiasRandomWalkCost(double accel_sigma, double gyro_sigma) : accel_sigma_(accel_sigma), gyro_sigma_(gyro_sigma) {}
    ...
    double accel_sigma_;  ///< Accelerometer bias random-walk sigma.
    double gyro_sigma_;   ///< Gyroscope bias random-walk sigma.
};
