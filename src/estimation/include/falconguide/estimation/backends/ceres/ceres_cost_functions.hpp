...
static const Eigen::Vector3d kGravityEnu{
    0.0, 0.0, -kGravityMps2}; ///< Gravity vector in ENU.
...
struct ImuPreintegrationCost {
  ...
  bool operator()(const T *pose_i, const T *vel_i, const T *bias_i,
    ...
    Eigen::Matrix<T, 3, 1> dba(
        bias_i[0] - T(preint_.linearisation_accel_bias_mps2.x()),
    ...
    Eigen::Matrix<T, 3, 1> dbg(
        bias_i[3] - T(preint_.linearisation_gyro_bias_radps.x()),
    ...
    Eigen::Matrix<T, 3, 1> dq_corr_axis =
        preint_.jacobian_dp_dba.block(3, 0, 3, 3).template cast<T>() * dbg;
    ...
    Eigen::Matrix<T, 3, 1> r_p =
        R_i.transpose() * (p_j - p_i - v_i * dt - T(0.5) * g_enu * dt * dt) -
        dp_corr;
    ...
    Eigen::Quaternion<T> q_err =
        (dq_hat * dq_corr).conjugate() * (q_i.conjugate() * q_j);
    ...
    const double info_scale =
        1.0 / std::max(1e-6, std::sqrt(preint_.covariance.trace() / 9.0));
  ...
  static ceres::CostFunction *Create(ImuPreintegration preint) {
    return new ceres::AutoDiffCostFunction<ImuPreintegrationCost, 9, 7, 3, 6, 7,
                                           3, 6>(
...
struct GnssPositionCost {
  ...
  template <typename T> bool operator()(const T *pose, T *residual) const {
    Eigen::Matrix<T, 3, 1> p(pose[0], pose[1], pose[2]);
    Eigen::Matrix<T, 3, 1> r =
        sqrt_info_.template cast<T>() * (p - meas_.template cast<T>());
    residual[0] = r(0);
    residual[1] = r(1);
    residual[2] = r(2);
    return true;
  ...
  /// @brief Creates a Ceres AutoDiff cost function for this factor.
  static ceres::CostFunction *Create(Eigen::Vector3d meas,
                                     Eigen::Matrix3d cov) {
...
struct GnssVelocityCost {
  ...
  template <typename T> bool operator()(const T *vel, T *residual) const {
    Eigen::Matrix<T, 3, 1> v(vel[0], vel[1], vel[2]);
    Eigen::Matrix<T, 3, 1> r =
        sqrt_info_.template cast<T>() * (v - meas_.template cast<T>());
    residual[0] = r(0);
    residual[1] = r(1);
    residual[2] = r(2);
    return true;
