...
class ImuPreintegrator {
 public:
  struct Params {
    double accel_noise_density_mps2_sqrthz{3.0e-3};
    double gyro_noise_density_radps_sqrthz{1.5e-4};
    double accel_random_walk_mps3_sqrthz{3.0e-5};
    double gyro_random_walk_radps2_sqrthz{2.0e-6};
  };

  ImuPreintegrator() = default;

  // Reset to a new integration window.
  void Reset(Eigen::Vector3d accel_bias_mps2,
             Eigen::Vector3d gyro_bias_radps,
             const core::Timestamp& t0,
             Params params = {});

  // Integrate one IMU sample.  Samples must arrive in chronological order.
  void Integrate(const core::ImuMeasurement& imu);

  // Finalise and return the preintegration result.
...
inline void ImuPreintegrator::Reset(Eigen::Vector3d accel_bias,
  ...
  delta_v_.setZero();
  cov_.setZero();
  J_dp_dba_.setZero(); J_dp_dbg_.setZero();
  J_dq_dbg_.setZero();
  J_dv_dba_.setZero(); J_dv_dbg_.setZero();
  delta_t_   = 0.0;
  has_prev_  = false;
  num_samples_ = 0;
}
