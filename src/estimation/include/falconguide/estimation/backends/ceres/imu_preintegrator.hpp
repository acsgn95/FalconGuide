...
class ImuPreintegrator {
   public:
    ...
    struct Params {
        double accel_noise_density_mps2_sqrthz{3.0e-3};  ///< Accelerometer white-noise density.
        double gyro_noise_density_radps_sqrthz{1.5e-4};  ///< Gyroscope white-noise density.
        double accel_random_walk_mps3_sqrthz{3.0e-5};    ///< Accelerometer bias random walk.
        double gyro_random_walk_radps2_sqrthz{2.0e-6};   ///< Gyroscope bias random walk.
    };
...
inline void ImuPreintegrator::Integrate(const core::ImuMeasurement &imu) {
    ...
    F.block<3, 3>(3, 3) = Eigen::Matrix3d::Identity();       // dθ / dθ (eye − Skew*dt below)
    ...
    const double qa = params_.accel_noise_density_mps2_sqrthz * params_.accel_noise_density_mps2_sqrthz;
    const double qg = params_.gyro_noise_density_radps_sqrthz * params_.gyro_noise_density_radps_sqrthz;
    ...
    J_dq_dbg_ = (Eigen::Matrix3d::Identity() - Skew(w_mid) * dt) * J_dq_dbg_ - Eigen::Matrix3d::Identity() * dt;
...
inline ImuPreintegration ImuPreintegrator::Finalize(const core::Timestamp &t1) const {
    ImuPreintegration p;
    ...
    p.jacobian_dp_dba.block<3, 3>(3, 0) = J_dq_dbg_;  // reuse field for dq/dbg
    ...
    p.linearisation_accel_bias_mps2 = core::Vec3<core::ImuFrame>(ba_.x(), ba_.y(), ba_.z());
    p.linearisation_gyro_bias_radps = core::Vec3<core::ImuFrame>(bg_.x(), bg_.y(), bg_.z());
