...
using StateVector = Eigen::Matrix<double, kStateSize, 1>;  ///< Fixed-size UKF state vector.
using StateCovariance = Eigen::Matrix<double, kStateSize,
...
static constexpr int kNumSigmaPoints = 2 * kStateSize + 1;  ///< Number of Merwe sigma points.

using SigmaMatrix = Eigen::Matrix<double, kStateSize,
...
struct MerweSigmaParams {
    ...
    double beta{2.0};    ///< Prior distribution parameter; 2 is optimal for Gaussian priors.
...
struct SigmaWeights {
    Eigen::Matrix<double, kNumSigmaPoints, 1> mean_weights;  ///< Weights used for mean recovery.
    Eigen::Matrix<double, kNumSigmaPoints, 1> cov_weights;   ///< Weights used for covariance recovery.
...
struct NominalState {
    ...
    Eigen::Quaterniond orientation_body_to_enu{Eigen::Quaterniond::Identity()};  ///< Body-to-ENU attitude mean.
...
struct UkfState {
    ...
    StateCovariance covariance{StateCovariance::Identity() * 1e-4};  ///< Current state covariance.
    SigmaMatrix sigma_points{SigmaMatrix::Zero()};                   ///< Most recently generated sigma points.
};
