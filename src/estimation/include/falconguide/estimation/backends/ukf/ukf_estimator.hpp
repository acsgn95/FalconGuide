...
struct UkfOptions {
  ...
  MerweSigmaParams
      sigma_params;        ///< Sigma-point spread and weighting parameters.
...
private:
  ...
  [[nodiscard]] StateCovariance
  BuildProcessNoise(const core::ImuMeasurement &imu, double dt_s) const;

  MeasurementUpdateReport
  ApplyMeasurementModel(IUkfMeasurementModel &model,
