...
struct GtsamImuPreintegration {
  ...
  Eigen::Vector3d delta_p{
      Eigen::Vector3d::Zero()}; ///< Preintegrated position delta.
  Eigen::Quaterniond delta_q{
      Eigen::Quaterniond::Identity()}; ///< Preintegrated rotation delta.
  Eigen::Vector3d delta_v{
      Eigen::Vector3d::Zero()}; ///< Preintegrated velocity delta.

  // Combined covariance [9×9: dp, dtheta, dv].
  Eigen::Matrix<double, 9, 9> covariance{
      Eigen::Matrix<double, 9, 9>::Zero()}; ///< Combined IMU covariance.
...
struct ISAM2Policy {
  ...
  double relinearise_threshold_velocity{
      0.001}; ///< Velocity relinearization threshold.
  double relinearise_threshold_bias{
      0.0001}; ///< Bias relinearization threshold.

  // Number of iterations of back-substitution per iSAM2 update.
  int relinearise_skip{
      10}; ///< Number of updates between relinearization checks.
...
struct FactorGraphState {
  ...
  std::vector<GtsamImuPreintegration>
      imu_factors; ///< IMU factors between consecutive nodes.
