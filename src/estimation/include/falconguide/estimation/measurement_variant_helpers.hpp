...
[[nodiscard]] inline const core::Timestamp &GetTimestamp(const SensorMeasurement &measurement) {
    return std::visit(
        [](const auto &typed_measurement) -> const core::Timestamp & { return core::GetTimestamp(typed_measurement); },
        measurement);
}
...
[[nodiscard]] inline core::MeasurementValidity GetValidity(const SensorMeasurement &measurement) {
    return std::visit([](const auto &typed_measurement) { return core::GetValidity(typed_measurement); }, measurement);
}
