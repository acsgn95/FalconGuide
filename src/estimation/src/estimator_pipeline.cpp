...
EstimatorPipeline::EstimatorPipeline(std::unique_ptr<INavigationEstimator> estimator, std::size_t queue_capacity)
    : estimator_(std::move(estimator)), queue_(queue_capacity) {}

EstimatorPipeline::~EstimatorPipeline() { Stop(); }

void EstimatorPipeline::RegisterObserver(INavigationObserver* observer) { observers_.push_back(observer); }
...
bool EstimatorPipeline::IsRunning() const { return running_.load(std::memory_order_relaxed); }
...
const INavigationEstimator& EstimatorPipeline::Estimator() const { return *estimator_; }
...
void EstimatorPipeline::RunLoop() {
    while (true) {
        ...
        if (report.result == EstimatorUpdateResult::Accepted) {
            ...
            if (state) {
                NotifyObservers(std::make_shared<const core::NavigationState>(std::move(*state)));
            }
        }
    }
}

void EstimatorPipeline::NotifyObservers(std::shared_ptr<const core::NavigationState> state) {
