...
NavigationSystem::NavigationSystem(const NavigationSystemConfig& config)
    : pipeline_(BuildEstimator(config)) {}
...
std::unique_ptr<INavigationEstimator> NavigationSystem::BuildEkfEstimator(
    const NavigationSystemConfig& config) {
  ...
  opts.enable_gnss_clock_state    =
      config.gnss.enabled &&
      config.gnss.mode == GnssIntegrationMode::TightlyCoupled;
  ...
  if (config.gnss.enabled) {
    if (config.gnss.mode == GnssIntegrationMode::LooselyCoupled)
      est->RegisterMeasurementModel(
          std::make_unique<ekf::GnssLooselyCoupled>(config.gnss.loosely_coupled));
    else
      est->RegisterMeasurementModel(
          std::make_unique<ekf::GnssTightlyCoupled>(config.gnss.tightly_coupled));
  }
  if (config.barometer.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::Barometer>(config.barometer.options));
  if (config.magnetometer.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::Magnetometer>(config.magnetometer.options));
  if (config.wheel_odometry.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::WheelOdometry>(config.wheel_odometry.options));
  if (config.airspeed.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::Airspeed>(config.airspeed.options));
  if (config.optical_flow.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::OpticalFlow>(config.optical_flow.options));
  if (config.radar_altimeter.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::RadarAltimeter>(config.radar_altimeter.options));
  if (config.range_finder.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::RangeFinder>(config.range_finder.options));
  if (config.dvl.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::Dvl>(config.dvl.options));
  if (config.echo_sounder.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::EchoSounder>(config.echo_sounder.options));
  if (config.external_pose.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::ExternalPose>(config.external_pose.options));
  if (config.external_velocity.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::ExternalVelocity>(config.external_velocity.options));
  if (config.external_odometry.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::ExternalOdometry>(config.external_odometry.options));
  if (config.aiding_solution.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::AidingSolution>(config.aiding_solution.options));
  if (config.star_tracker.enabled)
    est->RegisterMeasurementModel(
        std::make_unique<ekf::StarTracker>(config.star_tracker.options));
...
//
std::unique_ptr<INavigationEstimator> NavigationSystem::BuildUkfEstimator(
    const NavigationSystemConfig& config) {

  if (config.gnss.enabled &&
      config.gnss.mode == GnssIntegrationMode::TightlyCoupled) {
    FG_WARN("NavigationSystem | UKF does not support tightly coupled GNSS; "
            "falling back to loosely coupled");
  }

  ukf::UkfOptions opts;
  opts.sigma_params               = config.ukf_sigma_params;
  opts.imu_noise.accel_noise_density_mps2_per_sqrthz =
      config.imu.noise.accel_noise_density_mps2_per_sqrthz;
  opts.imu_noise.gyro_noise_density_radps_per_sqrthz =
      config.imu.noise.gyro_noise_density_radps_per_sqrthz;
  opts.imu_noise.accel_random_walk_mps3_per_sqrthz =
      config.imu.noise.accel_random_walk_mps3_per_sqrthz;
  opts.imu_noise.gyro_random_walk_radps2_per_sqrthz =
      config.imu.noise.gyro_random_walk_radps2_per_sqrthz;
  opts.dead_reckoning_threshold_s = config.dead_reckoning_threshold_s;
  opts.min_imu_dt_s               = config.min_imu_dt_s;
  opts.max_imu_dt_s               = config.max_imu_dt_s;
