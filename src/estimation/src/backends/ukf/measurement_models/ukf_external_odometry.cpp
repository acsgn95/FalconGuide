...
int UkfExternalOdometry::MeasurementDim(const SensorMeasurement& /*measurement*/) const {
    return (options_.use_position ? 3 : 0) + (options_.use_velocity ? 3 : 0) + (options_.use_orientation ? 3 : 0);
}

Eigen::VectorXd UkfExternalOdometry::Predict(const NominalState& sigma_state, const UkfUpdateContext& /*ctx*/) const {
    const int m =
        (options_.use_position ? 3 : 0) + (options_.use_velocity ? 3 : 0) + (options_.use_orientation ? 3 : 0);
    Eigen::VectorXd z(m);
    int row = 0;
...
std::optional<Eigen::VectorXd> UkfExternalOdometry::Observe(const SensorMeasurement& measurement,
    ...
    if (options_.use_orientation) {
        const Eigen::Quaterniond q_obs = (Eigen::Quaterniond(R_ecef_to_enu) * eo.orientation_body_to_ecef).normalized();
        z.segment<3>(row) = core::LogMapSo3(q_obs);
    }
    return z;
}

Eigen::MatrixXd UkfExternalOdometry::NoiseCovariance(const SensorMeasurement& /*measurement*/,
                                                     const UkfUpdateContext& /*ctx*/) const {
    const int m =
        (options_.use_position ? 3 : 0) + (options_.use_velocity ? 3 : 0) + (options_.use_orientation ? 3 : 0);
