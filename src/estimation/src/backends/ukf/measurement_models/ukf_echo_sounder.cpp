...
Eigen::VectorXd UkfEchoSounder::Predict(const NominalState& sigma_state, const UkfUpdateContext& /*ctx*/) const {
...
std::optional<Eigen::VectorXd> UkfEchoSounder::Observe(const SensorMeasurement& measurement,
                                                       const UkfUpdateContext& /*ctx*/) const {
    const auto& es = std::get<core::EchoSounderMeasurement>(measurement);
    if (es.validity != core::MeasurementValidity::Valid || es.depth_or_range_m < 0.0) return std::nullopt;
...
Eigen::MatrixXd UkfEchoSounder::NoiseCovariance(const SensorMeasurement& /*measurement*/,
                                                const UkfUpdateContext& /*ctx*/) const {
