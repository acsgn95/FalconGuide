...
std::tuple<bool, bool, bool> UkfAidingSolution::ActiveBlocks(const core::AidingSolution& aid) {
  return {aid.position_ecef_m.has_value(),
          aid.velocity_ecef_mps.has_value(),
...
Eigen::VectorXd UkfAidingSolution::Predict(
...
std::optional<Eigen::VectorXd> UkfAidingSolution::Observe(
...
Eigen::MatrixXd UkfAidingSolution::NoiseCovariance(
    const SensorMeasurement& measurement,
    const UkfUpdateContext& ctx) const {
  ...
  int row = 0;
  const Eigen::Matrix3d R_ecef_to_enu = ctx.ltp ? ctx.ltp->ecef_to_enu_rotation()
  ...
  if (has_vel) {
    ...
    if (s > 0.0) {
      ...
      R_meas.block<3, 3>(row, row) = R_ecef_to_enu * aid.velocity_covariance_ecef_mps2 * R_ecef_to_enu.transpose();
      if (R_meas.block<3, 3>(row, row).trace() < 1e-12)
