...
Eigen::VectorXd UkfExternalVelocity::Predict(
...
std::optional<Eigen::VectorXd> UkfExternalVelocity::Observe(
...
Eigen::MatrixXd UkfExternalVelocity::NoiseCovariance(
    const SensorMeasurement& measurement,
    const UkfUpdateContext& /*ctx*/) const {

  const auto& ev = std::get<core::ExternalVelocityMeasurement>(measurement);
  if (ev.covariance.block<3, 3>(0, 0).trace() > 1e-12)
    return ev.covariance.block<3, 3>(0, 0);
