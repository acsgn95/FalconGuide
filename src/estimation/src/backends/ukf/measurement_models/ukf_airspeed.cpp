...
Eigen::VectorXd UkfAirspeed::Predict(
...
std::optional<Eigen::VectorXd> UkfAirspeed::Observe(
    const SensorMeasurement& measurement,
    const UkfUpdateContext& /*ctx*/) const {

  const auto& asp = std::get<core::AirspeedMeasurement>(measurement);
  if (asp.validity != core::MeasurementValidity::Valid || asp.airspeed_mps < 0.0)
    return std::nullopt;
  return Eigen::VectorXd::Constant(1, asp.airspeed_mps);
}

Eigen::MatrixXd UkfAirspeed::NoiseCovariance(
    const SensorMeasurement& /*measurement*/,
    const UkfUpdateContext& /*ctx*/) const {
