...
Eigen::VectorXd UkfStarTracker::Predict(
...
std::optional<Eigen::VectorXd> UkfStarTracker::Observe(
...
Eigen::MatrixXd UkfStarTracker::NoiseCovariance(
    const SensorMeasurement& measurement,
    const UkfUpdateContext& /*ctx*/) const {
  ...
  if (full) {
    ...
    R.diagonal() << options_.sigma_roll_rad  * options_.sigma_roll_rad,
                    options_.sigma_pitch_rad * options_.sigma_pitch_rad,
                    options_.sigma_yaw_rad   * options_.sigma_yaw_rad;
    return R;
  }
