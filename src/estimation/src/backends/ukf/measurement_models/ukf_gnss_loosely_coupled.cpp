...
UkfGnssLooselyCoupled::UkfGnssLooselyCoupled(UkfGnssLooselyCoupledOptions options)
...
//
Eigen::VectorXd UkfGnssLooselyCoupled::Predict(
...
// ── Observe ───────────────────────────────────────────────────────────────────
std::optional<Eigen::VectorXd> UkfGnssLooselyCoupled::Observe(
...
// ── NoiseCovariance ───────────────────────────────────────────────────────────
Eigen::MatrixXd UkfGnssLooselyCoupled::NoiseCovariance(
    const SensorMeasurement& measurement,
    const UkfUpdateContext& ctx) const {

  const auto& gnss = std::get<core::GnssSolution>(measurement);
  const Eigen::Matrix3d R = ctx.ltp ? ctx.ltp->ecef_to_enu_rotation()
