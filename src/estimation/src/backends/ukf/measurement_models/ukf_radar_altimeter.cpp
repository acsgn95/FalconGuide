...
Eigen::VectorXd UkfRadarAltimeter::Predict(
...
std::optional<Eigen::VectorXd> UkfRadarAltimeter::Observe(
    const SensorMeasurement& measurement,
    const UkfUpdateContext& /*ctx*/) const {

  const auto& ralt = std::get<core::RadarAltimeterMeasurement>(measurement);
  if (ralt.validity == core::MeasurementValidity::Valid &&
      ralt.surface  != core::RadarAltimeterSurface::Invalid &&
      ralt.range_m  >= 0.0) {
...
Eigen::MatrixXd UkfRadarAltimeter::NoiseCovariance(
    const SensorMeasurement& /*measurement*/,
    const UkfUpdateContext& /*ctx*/) const {
