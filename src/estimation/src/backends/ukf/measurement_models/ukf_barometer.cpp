...
Eigen::VectorXd UkfBarometer::Predict(const NominalState& sigma_state, const UkfUpdateContext& /*ctx*/) const {
    // h(σ) = z_enu (altitude above LTP origin)
    return Eigen::VectorXd::Constant(1, sigma_state.position_enu_m.eigen().z());
}

std::optional<Eigen::VectorXd> UkfBarometer::Observe(const SensorMeasurement& measurement,
...
Eigen::MatrixXd UkfBarometer::NoiseCovariance(const SensorMeasurement& /*measurement*/,
                                              const UkfUpdateContext& /*ctx*/) const {
