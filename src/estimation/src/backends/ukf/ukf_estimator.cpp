...
UkfEstimator::UkfEstimator(UkfOptions options)
    : options_(std::move(options)), weights_(SigmaWeights::Compute(options_.sigma_params)), imu_buffer_(2000) {}
...
EstimatorInfo UkfEstimator::Info() const { return {EstimatorBackend::Custom, "UkfEstimator", "1.0"}; }

const EstimatorOptions& UkfEstimator::Options() const { return options_.base; }
bool UkfEstimator::IsInitialized() const {
...
MeasurementUpdateReport UkfEstimator::AddMeasurement(const SensorMeasurement& measurement) {
    ...
    if (const auto* imu = std::get_if<core::ImuMeasurement>(&measurement)) {
        ...
        if (r == core::BufferInsertResult::RejectedOutOfOrder || r == core::BufferInsertResult::RejectedNotComparable) {
    ...
    if (!initialised_) {
        if (const auto* gnss = std::get_if<core::GnssSolution>(&measurement)) {
            ...
            return {ok ? EstimatorUpdateResult::Accepted : EstimatorUpdateResult::Rejected, "UkfGnssLooselyCoupled"};
        }
        ...
        if (report.result == EstimatorUpdateResult::Accepted) {
            ...
            if (report.correction_norm) {
                ...
                if (*report.correction_norm > 10.0) {
                    FG_WARN("UKF | {} large correction={:.4f}m — possible sensor outlier", report.model_name,
...
static NominalState PropagateOnePoint(const NominalState& s, const core::ImuMeasurement& imu, double dt_s) {
    ...
    NominalState out;
    out.position_enu_m = core::Vec3<core::EnuFrame>(s.position_enu_m.eigen() + s.velocity_enu_mps.eigen() * dt_s +
                                                    0.5 * a_enu * dt_s * dt_s);
...
void UkfEstimator::GenerateSigmaPoints() {
    const double n = kStateSize;
    const double lambda =
        options_.sigma_params.alpha * options_.sigma_params.alpha * (n + options_.sigma_params.kappa) - n;
...
void UkfEstimator::PropagateImu(const core::ImuMeasurement& imu, double dt_s) {
    ...
    for (int iter = 0; iter < 5; ++iter) {
        ...
        for (int i = 0; i < kNumSigmaPoints; ++i) {
            mean_dtheta +=
                weights_.mean_weights(i) * core::LogMapSo3(q_mean.conjugate() * propagated[i].orientation_body_to_enu);
        }
...
//
StateCovariance UkfEstimator::BuildProcessNoise(const core::ImuMeasurement& imu, double dt_s) const {
...
//
MeasurementUpdateReport UkfEstimator::ApplyMeasurementModel(IUkfMeasurementModel& model,
    ...
    nom.orientation_body_to_enu = (nom.orientation_body_to_enu * core::ExpMapSo3(delta_x.segment<3>(6))).normalized();
...
bool UkfEstimator::TryInitialiseFromGnss(const core::GnssSolution& gnss) {
    ...
    FG_INFO("UKF | initialized — LTP origin lat={:.6f}° lon={:.6f}° alt={:.1f}m",
            origin_lla.latitude_rad * (180.0 / M_PI), origin_lla.longitude_rad * (180.0 / M_PI), origin_lla.altitude_m);
    return true;
}
...
core::NavigationState UkfEstimator::BuildNavigationState() const {
    ...
    if (local_tangent_plane_) {
        ...
        out.velocity_ecef_mps =
            Vec3<EcefFrame>(local_tangent_plane_->ecef_to_enu_rotation().transpose() * nom.velocity_enu_mps.eigen());
    }
    ...
    if (last_aiding_timestamp_ && nom.timestamp.has_steady && last_aiding_timestamp_->has_steady) {
        ...
        out.status = (age && age->seconds() > options_.dead_reckoning_threshold_s) ? NavigationStatus::DeadReckoning
                                                                                   : NavigationStatus::Nominal;
    } else {
        out.status = NavigationStatus::DeadReckoning;
    }

    out.mode = measurement_models_.empty() ? EstimatorMode::InertialOnly : EstimatorMode::MultiSensorFusion;
