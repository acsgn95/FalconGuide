...
static gtsam::imuBias::ConstantBias ToGtsamBias(const Eigen::Vector3d& ba, const Eigen::Vector3d& bg) {
    return {ba, bg};
}
...
class GtsamFactorGraphEstimator::Impl {
   public:
    explicit Impl(GtsamOptions opts) : opts_(std::move(opts)) {
        ...
        if (!opts_.isam2_policy.use_cholesky) params.factorization = gtsam::ISAM2Params::QR;
    ...
    MeasurementUpdateReport AddImu(const core::ImuMeasurement& imu) {
        ...
        if (preint_) {
            preint_->integrateMeasurement(
                imu.specific_force_mps2.eigen(), imu.angular_rate_radps.eigen(),
                prev_imu_ts_.has_steady ? (imu.timestamp.steady - prev_imu_ts_.steady).seconds() : 0.01);
        }
    ...
    MeasurementUpdateReport InitFromGnss(const core::GnssSolution& gnss) {
        ...
        // Prior noise models
        auto prior_noise_pose =
            gtsam::noiseModel::Diagonal::Sigmas((gtsam::Vector6() << 0.01, 0.01, 0.01, 10.0, 10.0, 10.0).finished());
        ...
        auto gnss_noise = gtsam::noiseModel::Gaussian::Covariance(
            ltp_.ecef_to_enu_rotation() * gnss.position_covariance_ecef_m2 * ltp_.ecef_to_enu_rotation().transpose());
        new_factors_.emplace_shared<gtsam::GPSFactor>(X(idx), gtsam::Point3(pos_enu), gnss_noise);
        ...
        preint_ = std::make_shared<gtsam::PreintegratedCombinedMeasurements>(preint_params_, prev_bias_);
        prev_pose_idx_ = idx;
    ...
    MeasurementUpdateReport NewNode(const core::GnssSolution& gnss) {
        ...
        const Eigen::Vector3d pos_enu = R_enu * (gnss.position_ecef_m.eigen() - ltp_.origin_ecef().eigen());
        ...
        // Combined IMU factor
        new_factors_.emplace_shared<gtsam::CombinedImuFactor>(X(prev_idx), V(prev_idx), X(cur_idx), V(cur_idx),
        ...
        new_factors_.emplace_shared<gtsam::GPSFactor>(X(cur_idx), gtsam::Point3(pos_enu), gnss_noise);
...
GtsamFactorGraphEstimator::GtsamFactorGraphEstimator(GtsamOptions options)
    : options_(std::move(options)), impl_(std::make_unique<Impl>(options_)), graph_(impl_->graph_) {}
...
const EstimatorOptions& GtsamFactorGraphEstimator::Options() const { return options_.base; }
...
MeasurementUpdateReport GtsamFactorGraphEstimator::AddMeasurement(const SensorMeasurement& measurement) {
    std::unique_lock lk(impl_->mutex_);
...
EstimatorUpdateResult GtsamFactorGraphEstimator::ProcessUntil(const core::Timestamp& /*ts*/) {
...
const FactorGraphState& GtsamFactorGraphEstimator::GraphState() const { return impl_->graph_; }
...
GtsamFactorGraphEstimator::GtsamFactorGraphEstimator(GtsamOptions) {
    throw std::runtime_error(
        "GtsamFactorGraphEstimator: built without GTSAM support. "
...
EstimatorUpdateResult GtsamFactorGraphEstimator::ProcessUntil(const core::Timestamp&) {
    return EstimatorUpdateResult::BackendError;
}
void GtsamFactorGraphEstimator::Reset() {}
const FactorGraphState& GtsamFactorGraphEstimator::GraphState() const {
    static FactorGraphState dummy;
    return dummy;
}
