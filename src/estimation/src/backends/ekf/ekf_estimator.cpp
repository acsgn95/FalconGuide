...
EkfEstimator::EkfEstimator(EkfOptions options) : options_(std::move(options)), imu_buffer_(/*max_size=*/2000) {
...
EstimatorInfo EkfEstimator::Info() const { return {EstimatorBackend::Ekf, "EkfEstimator", "1.0"}; }

const EstimatorOptions& EkfEstimator::Options() const { return options_.base; }
...
MeasurementUpdateReport EkfEstimator::AddMeasurement(const SensorMeasurement& measurement) {
    ...
    if (!initialised_) {
        if (const auto* gnss = std::get_if<core::GnssSolution>(&measurement)) {
            ...
            return {ok ? EstimatorUpdateResult::Accepted : EstimatorUpdateResult::Rejected, "GnssLooselyCoupled"};
        }
    ...
    for (auto& model : measurement_models_) {
        ...
        const auto result =
            model->Apply(state_.nominal, state_.error_state, state_.covariance, state_.layout, measurement, ctx);
...
void EkfEstimator::PropagateImu(const core::ImuMeasurement& imu, const double dt_s) {
    ...
    // ── Nominal state ─────────────────────────────────────────────────────────
    nom.position_enu_m =
        Vec3<EnuFrame>(nom.position_enu_m.eigen() + nom.velocity_enu_mps.eigen() * dt_s + 0.5 * a_enu * dt_s * dt_s);
    nom.velocity_enu_mps = Vec3<EnuFrame>(nom.velocity_enu_mps.eigen() + a_enu * dt_s);
    nom.orientation_body_to_enu = (nom.orientation_body_to_enu * ExpMapSo3(omega * dt_s)).normalized();
