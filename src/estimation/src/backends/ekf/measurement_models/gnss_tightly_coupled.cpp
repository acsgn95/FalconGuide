...
GnssTightlyCoupled::GnssTightlyCoupled(GnssTightlyCoupledOptions options)
...
double GnssTightlyCoupled::CarrierFrequencyHz(core::GnssSignal signal) {
  switch (signal) {
    case core::GnssSignal::L1:  return 1'575.42e6;
    case core::GnssSignal::L2:  return 1'227.60e6;
    case core::GnssSignal::L5:  return 1'176.45e6;
    case core::GnssSignal::E1:  return 1'575.42e6;
    case core::GnssSignal::E5a: return 1'176.45e6;
    case core::GnssSignal::E5b: return 1'207.14e6;
    case core::GnssSignal::B1:  return 1'561.098e6;
    case core::GnssSignal::B2:  return 1'207.14e6;
    default:                     return 1'575.42e6;  // fallback to GPS L1
  }
}
...
//
double GnssTightlyCoupled::SagnacCorrectionM(
    const Eigen::Vector3d& pos_sv,
    const Eigen::Vector3d& pos_rcv) {
  return (kEarthRotationRps / kSpeedOfLightMps) *
         (pos_sv.x() * pos_rcv.y() - pos_sv.y() * pos_rcv.x());
}
...
//
bool GnssTightlyCoupled::ScalarUpdate(
  ...
  // Innovation gate
  if (options_.innovation_gate_sigma > 0.0 &&
      std::abs(innovation) > options_.innovation_gate_sigma * std::sqrt(S)) {
    return false;
  }
...
EstimatorUpdateResult GnssTightlyCoupled::Apply(
  ...
  for (const auto& sv_obs : epoch.sv_observations) {
    ...
    // ── Pseudorange update ─────────────────────────────────────────────────
    {
      ...
      const double rho_pred = range
                              + sagnac
                              + clk_bias_m
                              - sv_obs.satellite.clock_bias_s   * kSpeedOfLightMps
                              + sv_obs.satellite.ionospheric_delay_m
                              + sv_obs.satellite.tropospheric_delay_m;
    ...
    if (sv_obs.raw.doppler_hz) {
      ...
      const double range_rate_pred = e.dot(v_sv - vel_rcv_ecef)
                                     + clk_drift_mps
                                     - sv_obs.satellite.clock_drift_sps * kSpeedOfLightMps;
