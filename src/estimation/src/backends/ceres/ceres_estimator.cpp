...
struct PoseBlock {
  ...
  void Set(const Eigen::Vector3d& p, const Eigen::Quaterniond& q) {
    ...
    data[3]=q.x(); data[4]=q.y(); data[5]=q.z(); data[6]=q.w();
  }
...
struct WindowKeyframe {
  ...
  void SetV(const Eigen::Vector3d& v) { vel[0]=v.x(); vel[1]=v.y(); vel[2]=v.z(); }
...
class CeresSlidingWindowEstimator::Impl {
 public:
  ...
  MeasurementUpdateReport AddGnss(const core::GnssSolution& gnss) {
    ...
    const Eigen::Vector3d pos_enu = ltp_.ecef_to_enu_rotation() *
        (gnss.position_ecef_m.eigen() - ltp_.origin_ecef().eigen());
  ...
  MeasurementUpdateReport InitFromGnss(const core::GnssSolution& gnss) {
    ...
    const ImuPreintegrator::Params pp{
        opts_.base.enable_sensor_health_reporting ? 3e-3 : 3e-3};
    preint_.Reset(Eigen::Vector3d::Zero(), Eigen::Vector3d::Zero(),
                  gnss.timestamp, ImuPreintegrator::Params{});
    // Re-play buffered IMU into the preintegrator
    for (const auto& i : imu_buffer_) {
      if (i.timestamp.steady >= gnss.timestamp.steady)
        preint_.Integrate(i);
    }
    preint_active_ = true;
  ...
  MeasurementUpdateReport UpdateLatestGnss(const core::GnssSolution& gnss,
    ...
    problem.AddResidualBlock(
        GnssPositionCost::Create(pos_enu, cov_enu), nullptr, kf.pose.data);
    problem.AddResidualBlock(
        GnssVelocityCost::Create(
            R_enu * gnss.velocity_ecef_mps.eigen(),
            R_enu * gnss.velocity_covariance_ecef_m2ps2 * R_enu.transpose()),
        nullptr, kf.vel);

    // Quaternion parametrisation
    problem.SetParameterization(kf.pose.data,
  ...
  MeasurementUpdateReport CreateKeyframe(const core::GnssSolution& gnss,
                                         const Eigen::Vector3d& pos_enu) {
    ...
    const Eigen::Vector3d p_pred = prev.P() + prev.V() * dt + 0.5 * g_enu * dt * dt +
                                   R_prev * preint.delta_p;
  ...
  void Optimise(const core::GnssSolution& gnss, const Eigen::Vector3d& pos_enu) {
    ...
    for (std::size_t i = 0; i + 1 < n && i < imu_factors_.size(); ++i) {
      problem.AddResidualBlock(
          ImuPreintegrationCost::Create(imu_factors_[i]),
          nullptr,
          window_[i].pose.data, window_[i].vel, window_[i].bias,
          window_[i+1].pose.data, window_[i+1].vel, window_[i+1].bias);
    }
    ...
    for (std::size_t i = 0; i + 1 < n; ++i) {
      problem.AddResidualBlock(
          BiasRandomWalkCost::Create(ba_sigma, bg_sigma),
          nullptr,
          window_[i].bias, window_[i+1].bias);
    }

    // ── GNSS factors on latest keyframe ───────────────────────────────────
    auto& latest = window_.back();
    const Eigen::Matrix3d cov_enu =
        R_enu * gnss.position_covariance_ecef_m2 * R_enu.transpose();
    ...
    problem.AddResidualBlock(
        GnssPositionCost::Create(pos_enu, cov_enu), pos_loss, latest.pose.data);
    problem.AddResidualBlock(
        GnssVelocityCost::Create(R_enu * gnss.velocity_ecef_mps.eigen(), vel_cov_enu),
        vel_loss, latest.vel);
    ...
    if (opts_.solver_strategy == SolverStrategy::FastOneIteration)
      sopts.max_num_iterations = 1;
...
MeasurementUpdateReport CeresSlidingWindowEstimator::AddMeasurement(
...
EstimatorUpdateResult CeresSlidingWindowEstimator::ProcessUntil(
...
CeresSlidingWindowEstimator::CeresSlidingWindowEstimator(CeresOptions) {
  throw std::runtime_error("CeresSlidingWindowEstimator: built without Ceres support. "
...
EstimatorUpdateResult CeresSlidingWindowEstimator::ProcessUntil(const core::Timestamp&) { return EstimatorUpdateResult::BackendError; }
void CeresSlidingWindowEstimator::Reset() {}
const SlidingWindowState& CeresSlidingWindowEstimator::WindowState() const {
  static SlidingWindowState dummy; return dummy;
}
